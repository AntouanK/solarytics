        ...
        case dateInt of
            ...
            Ok dateInt ->
                ...
                in
                    let
                        month =
                            ((dateInt - date) // 100) % 100
                    in
                        let
                            year =
                                2000 + (((dateInt - (month * 100) - date)) // 10000)
                        in
                            Just
                                { day = date
...
datesToMonthdata dates =
    ...
    in
        ...
        in
            let
                dateInts =
                    List.map toIntWithDefault (List.map .date dates)
            in
                let
                    dict =
                        List.foldl sortDateToMonthData (Dict.fromList []) dateInts
                in
